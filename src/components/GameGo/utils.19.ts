import { Board, Player, Position, DIRECTIONS, BOARD_SIZE } from './types.19';
...
export const calculateScore = (board: Board): { black: number, white: number, territory: Board } => {
  ...
  for (let row = 0; row < BOARD_SIZE; row++) {
    for (let col = 0; col < BOARD_SIZE; col++) {
      if (board[row][col] === null && !visited.has(`${row},${col}`)) {
        ...
        // 如果空点区域只被一种颜色包围，则标记为该颜色的领地
        if (surroundingColors.size === 1) {
          const territoryColor = Array.from(surroundingColors)[0];
          emptyArea.forEach(([r, c]) => {
            territory[r][c] = territoryColor;
          });
        }
      }
    }
  ...
  for (let row = 0; row < BOARD_SIZE; row++) {
    for (let col = 0; col < BOARD_SIZE; col++) {
      ...
      if (stone === 'black' || territoryOwner === 'black') black++;
      else if (stone === 'white' || territoryOwner === 'white') white++;
    }
  }
